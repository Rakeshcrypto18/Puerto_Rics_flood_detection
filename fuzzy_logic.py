...
df_small_size = df_change_points.groupby('poly_id').size().reset_index(name='point_counts')
...
to_gdf= pd.merge(df_change_points,df_small_size, on='poly_id', how = 'left')
to_gdf = to_gdf.drop_duplicates(subset=['poly_id'], keep='first')
to_gdf.columns
to_gdf
